...
namespace torch_ext::sage_attention3
{
...
void scaled_fp4_quant(torch::Tensor const& input,
  ...
  TORCH_CHECK(output.get_device() == input.get_device()
          && output_sf.get_device() == input.get_device(),
      "SageAttention3 FP4 quantization tensors must be on the same CUDA device");
...
std::tuple<torch::Tensor, torch::Tensor> scaled_fp4_quantize(torch::Tensor const& input, int64_t tensor_layout)
{
  CHECK_DIMS(input, 4);
  int64_t const batch_size = input.size(0);
  int64_t const head_dim = input.size(3);
  TORCH_CHECK(head_dim == 64 || head_dim == 128, "SageAttention3 FP4 quantization supports head dimensions 64 and 128");

  torch::Tensor packed_fp4;
  torch::Tensor fp8_scale;
  if (tensor_layout == 0) {
    int64_t const num_tokens = input.size(1);
    int64_t const num_heads = input.size(2);
    packed_fp4 = torch::empty({batch_size, num_tokens, num_heads, head_dim / 2}, input.options().dtype(torch::kUInt8));
    fp8_scale = torch::empty({batch_size, num_tokens, num_heads, head_dim / 16},
        input.options().dtype(torch::kFloat8_e4m3fn));
  } else if (tensor_layout == 1) {
    int64_t const num_heads = input.size(1);
    int64_t const num_tokens = input.size(2);
    packed_fp4 = torch::empty({batch_size, num_heads, num_tokens, head_dim / 2}, input.options().dtype(torch::kUInt8));
    fp8_scale = torch::empty({batch_size, num_heads, num_tokens, head_dim / 16},
        input.options().dtype(torch::kFloat8_e4m3fn));
  } else {
    TORCH_CHECK(false, "tensor_layout must be 0 ([B, S, H, D]) or 1 ([B, H, S, D])");
  }

  scaled_fp4_quant(input, packed_fp4, fp8_scale, static_cast<int>(tensor_layout));
  return std::make_tuple(packed_fp4, fp8_scale);
}

std::tuple<torch::Tensor, torch::Tensor> scaled_fp4_permute_quantize(torch::Tensor const& input, int64_t tensor_layout)
{
  CHECK_DIMS(input, 4);
  ...
  if (tensor_layout == 0) {
    int64_t const num_tokens = input.size(1);
    int64_t const num_heads = input.size(2);
    int64_t const padded_tokens = (num_tokens + block_size - 1) / block_size * block_size;
    packed_fp4 = torch::empty({batch_size, padded_tokens, num_heads, head_dim / 2}, input.options().dtype(torch::kUInt8));
    fp8_scale = torch::empty({batch_size, padded_tokens, num_heads, head_dim / 16},
        input.options().dtype(torch::kFloat8_e4m3fn));
  } else if (tensor_layout == 1) {
    int64_t const num_heads = input.size(1);
    int64_t const num_tokens = input.size(2);
    int64_t const padded_tokens = (num_tokens + block_size - 1) / block_size * block_size;
    packed_fp4 = torch::empty({batch_size, num_heads, padded_tokens, head_dim / 2}, input.options().dtype(torch::kUInt8));
    fp8_scale = torch::empty({batch_size, num_heads, padded_tokens, head_dim / 16},
        input.options().dtype(torch::kFloat8_e4m3fn));
  } else {
    TORCH_CHECK(false, "tensor_layout must be 0 ([B, S, H, D]) or 1 ([B, H, S, D])");
  }

  scaled_fp4_quant_permute(input, packed_fp4, fp8_scale, static_cast<int>(tensor_layout));
  return std::make_tuple(packed_fp4, fp8_scale);
}

std::tuple<torch::Tensor, torch::Tensor> scaled_fp4_transpose_quantize(torch::Tensor const& input, int64_t tensor_layout)
{
  CHECK_DIMS(input, 4);
  constexpr int64_t block_size = 128;
  int64_t const batch_size = input.size(0);
  ...
  if (tensor_layout == 0) {
    ...
    int64_t const num_heads = input.size(2);
    int64_t const padded_tokens = (num_tokens + block_size - 1) / block_size * block_size;
    packed_fp4 = torch::empty({batch_size, head_dim, num_heads, padded_tokens / 2}, input.options().dtype(torch::kUInt8));
    fp8_scale = torch::empty({batch_size, head_dim, num_heads, padded_tokens / 16},
        input.options().dtype(torch::kFloat8_e4m3fn));
  } else if (tensor_layout == 1) {
    int64_t const num_heads = input.size(1);
    int64_t const num_tokens = input.size(2);
    int64_t const padded_tokens = (num_tokens + block_size - 1) / block_size * block_size;
    packed_fp4 = torch::empty({batch_size, num_heads, head_dim, padded_tokens / 2}, input.options().dtype(torch::kUInt8));
    fp8_scale = torch::empty({batch_size, num_heads, head_dim, padded_tokens / 16},
        input.options().dtype(torch::kFloat8_e4m3fn));
  } else {
    TORCH_CHECK(false, "tensor_layout must be 0 ([B, S, H, D]) or 1 ([B, H, S, D])");
  }

  scaled_fp4_quant_trans(input, packed_fp4, fp8_scale, static_cast<int>(tensor_layout));
  return std::make_tuple(packed_fp4, fp8_scale);
}
...
{
  m.def("sage_attention3_scaled_fp4_quantize(Tensor input, int tensor_layout) -> (Tensor, Tensor)");
  m.def("sage_attention3_scaled_fp4_permute_quantize(Tensor input, int tensor_layout) -> (Tensor, Tensor)");
  m.def("sage_attention3_scaled_fp4_transpose_quantize(Tensor input, int tensor_layout) -> (Tensor, Tensor)");
}

TORCH_LIBRARY_IMPL(trtllm, CUDA, m)
{
  m.impl("sage_attention3_scaled_fp4_quantize",
      TORCH_FN(tensorrt_llm::torch_ext::sage_attention3::scaled_fp4_quantize));
  m.impl("sage_attention3_scaled_fp4_permute_quantize",
      TORCH_FN(tensorrt_llm::torch_ext::sage_attention3::scaled_fp4_permute_quantize));
  m.impl("sage_attention3_scaled_fp4_transpose_quantize",
      TORCH_FN(tensorrt_llm::torch_ext::sage_attention3::scaled_fp4_transpose_quantize));
}
