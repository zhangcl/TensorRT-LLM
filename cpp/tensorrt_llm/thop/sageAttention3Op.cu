...
mha_fwd(at::Tensor const& q,         // batch_size x seqlen_q x num_heads x (head_size // 2)
        ...
        const at::Tensor &delta_s,
        int64_t unpadded_k,
        std::optional<at::Tensor> const& out_,             // batch_size x seqlen_q x num_heads x head_size
    ...
    CHECK_DEVICE(q); CHECK_DEVICE(k); CHECK_DEVICE(v); CHECK_DEVICE(delta_s);
    ...
    TORCH_CHECK(is_sm100 || is_sm120 || is_sm121, "SageAttention3 only supports Blackwell SM100/SM120/SM121 GPUs.");
    ...
    TORCH_CHECK(sfq_dtype == torch::kFloat8_e4m3fn, "q scale dtype must be float8_e4m3fn");
    TORCH_CHECK(sfk.dtype() == sfq_dtype, "query and key scales must have the same dtype");
    TORCH_CHECK(sfv.dtype() == sfq_dtype, "query and value scales must have the same dtype");
    CHECK_DEVICE(sfq); CHECK_DEVICE(sfk); CHECK_DEVICE(sfv);
    TORCH_CHECK(k.get_device() == q.get_device() && v.get_device() == q.get_device()
            && sfq.get_device() == q.get_device() && sfk.get_device() == q.get_device()
            && sfv.get_device() == q.get_device() && delta_s.get_device() == q.get_device(),
        "SageAttention3 tensors must be on the same CUDA device");
    ...
    TORCH_CHECK(unpacked_head_size < 256, "SageAttention3 forward currently supports head dimensions 64 and 128");
    ...
    TORCH_CHECK(seqlen_q % 128 == 0 && seqlen_k % 128 == 0,
        "SageAttention3 packed Q/K/V sequence lengths must be padded to a multiple of 128");
    ...
    if (per_block_mean) {
        ...
        CHECK_SHAPE(delta_s, batch_size, num_heads, 1, seqlen_k);
    }
    CHECK_SHAPE(sfq, batch_size, num_heads, seqlen_q, unpacked_head_size / 16);
    CHECK_SHAPE(sfk, batch_size, num_heads_k, seqlen_k, unpacked_head_size / 16);
    CHECK_SHAPE(sfv, batch_size, num_heads_k, unpacked_head_size, seqlen_k / 16);
    ...
    at::Tensor out = out_.has_value()
        ? out_.value()
        : torch::empty({batch_size, num_heads, seqlen_q, unpacked_head_size}, opts.dtype(dtype));
    CHECK_DEVICE(out);
    CHECK_CONTIGUOUS(out);
    CHECK_SHAPE(out, batch_size, num_heads, seqlen_q, unpacked_head_size);
    TORCH_CHECK(out.get_device() == q.get_device(), "out must be on the same CUDA device as q");
    TORCH_CHECK(out.scalar_type() == dtype, "out must match the requested SageAttention3 output dtype");
    TORCH_CHECK(unpadded_k >= 0 && unpadded_k <= seqlen_k, "unpadded_k must be in [0, seqlen_k]");
...
{
    m.def(
        "sage_attention3_fwd(Tensor q, Tensor k, Tensor v, Tensor sfq, Tensor sfk, Tensor sfv, Tensor delta_s, "
        "int unpadded_k, Tensor? out, float softmax_scale, bool is_causal, bool per_block_mean, bool is_bf16) "
        "-> (Tensor, Tensor)");
}

TORCH_LIBRARY_IMPL(trtllm, CUDA, m)
{
    m.impl("sage_attention3_fwd", TORCH_FN(tensorrt_llm::torch_ext::sage_attention3::mha_fwd));
}
